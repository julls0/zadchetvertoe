...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save'])) {
    ...
    $messages[] = 'Ваши результаты в системе хранения';
  }
  ...
  if ($errors['name']) {
    ...
    $messages[] = '<div class="error"> Неверный формат </div>';
  }
  if ($errors['email']) {
  ...
  if ($errors['year']) {
    ...
    $messages[] = '<div class="error"> Введитее год </div>';
  }
  if ($errors['radio-1']) {
  ...
  if ($errors['check-1']) {
    ...
    $messages[] = '<div class="error"> Согласие на обработку данных </div>';
  }
